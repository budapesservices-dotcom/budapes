...
export default function App() {
  ...
  useEffect(() => {
    // Pastikan loading ditampilkan saat page load
    const timer = setTimeout(() => {
      setShowLoading(false);
    }, 5000); // Fallback jika onComplete tidak dipanggil
  ...
  return (
    <>
      <AnimatePresence>
        {showLoading && (
          <SharedLoading
            reverse={true} // Mode Menghapus
            onComplete={() => setShowLoading(false)} // Setelah terhapus, munculkan konten
          />
        )}
      </AnimatePresence>
